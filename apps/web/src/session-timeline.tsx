...
import { duration, rawJson, timelineItems, toolStatus, type RuntimeTurn, type TimelineEvent, type TimelinePart } from './timeline-model.js'
  ...
  return (
    <section className="space-y-5 pb-6">
      ...
      {items.map((item) => item.kind === 'system'
        ? <TimelinePartView isRunning={isRunning} key={item.id} part={item.part} />
        : <TurnItem isRunning={isRunning} item={item} key={item.id} />)}
    </section>
  )
}
...
function TurnItem({ isRunning, item }: { isRunning: boolean; item: Extract<ReturnType<typeof timelineItems>[number], { kind: 'turn' }> }) {
  return (
...
function TimelinePartView({ isRunning, part }: { isRunning: boolean; part: TimelinePart }) {
  if (part.kind === 'assistant_text') return <article className="max-w-3xl"><pre className="whitespace-pre-wrap break-words font-sans text-[15px] leading-7">{part.text}</pre><RawEvent events={part.events} /></article>
  if (part.kind === 'thinking') return <details className="rounded-md border bg-muted/30 px-3 py-2 text-sm" open={isRunning}><summary className="cursor-pointer font-medium text-muted-foreground">思考中</summary><pre className="mt-2 max-h-80 overflow-auto whitespace-pre-wrap break-words font-sans text-xs leading-6">{part.text}</pre><RawEvent events={part.events} /></details>
  ...
  if (part.kind === 'file_change') return <FileChangePart part={part} />
  if (part.kind === 'system') return <SystemPart part={part} />
  return <NativePart part={part} />
}
...
function ToolPart({ isRunning, part }: { isRunning: boolean; part: Extract<TimelinePart, { kind: 'tool' }> }) {
  ...
  return <details className="rounded-md border bg-card px-3 py-2 text-sm" open={isRunning && status === '正在执行'}><summary className="cursor-pointer font-medium">{status === '正在执行' ? '正在执行：' : ''}{part.toolName}<span className="ml-2 text-xs text-muted-foreground">{status}{elapsed === undefined ? '' : ` · ${formatDuration(elapsed)}`}</span></summary><RawEvent events={part.events} /></details>
}
...
function FileChangePart({ part }: { part: Extract<TimelinePart, { kind: 'file_change' }> }) {
  ...
  return <details className="rounded-md border bg-card px-3 py-2 text-sm"><summary className="cursor-pointer font-medium">文件变更<span className="ml-2 text-xs text-muted-foreground">{changes.length ? `${changes.length} 项` : 'Pi 已报告变更'}</span></summary>{changes.length > 0 && <pre className="mt-3 max-h-80 overflow-auto whitespace-pre-wrap break-words text-xs">{JSON.stringify(changes, null, 2)}</pre>}<RawEvent events={part.events} /></details>
}

function SystemPart({ part }: { part: Extract<TimelinePart, { kind: 'system' }> }) {
  return <div className="border-l-2 border-muted px-3 py-1 text-xs text-muted-foreground">{systemLabel(part.events.at(-1))}<RawEvent events={part.events} /></div>
}

function NativePart({ part }: { part: Extract<TimelinePart, { kind: 'native' }> }) {
  return <details className="rounded-md border border-dashed px-3 py-2 text-sm"><summary className="cursor-pointer text-muted-foreground">Pi 原生事件：{eventName(part.events.at(-1))}</summary><RawEvent events={part.events} alwaysOpen /></details>
}

function RawEvent({ alwaysOpen = false, events }: { alwaysOpen?: boolean; events: TimelineEvent[] }) {
  const content = <pre className="mt-2 max-h-96 overflow-auto whitespace-pre-wrap break-words rounded bg-muted/50 p-2 text-xs leading-5">{rawJson(events)}</pre>
  return alwaysOpen ? content : <details className="mt-2 text-xs text-muted-foreground"><summary className="cursor-pointer">原始 JSON</summary>{content}</details>
}
...
function eventName(event: TimelineEvent | undefined) { return typeof event?.event.type === 'string' ? event.event.type : 'unknown' }
function systemLabel(event: TimelineEvent | undefined) {
  const type = eventName(event)
  if (type === 'model_change') return '模型已切换'
  if (type === 'thinking_level_change' || type === 'thinking_level_changed') return 'Thinking 等级已变更'
  if (type.startsWith('compaction')) return '上下文压缩'
  if (type === 'turn_start') return 'Pi 开始执行'
  if (type === 'turn_end' || type === 'agent_settled' || type === 'agent_end') return 'Pi 执行结束'
  if (type === 'agent_start') return 'Pi 正在执行'
  if (type === 'aborted') return 'Pi 已中止'
  return `Pi 系统事件：${type}`
}
